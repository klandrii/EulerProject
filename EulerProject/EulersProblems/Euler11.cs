...
namespace EulerProblems
{
    internal class Euler11
    {
        ...
        private int[,] grid =
        {
            ...
            } //Later the grid input method can be changed for now i will use this template
            ,
        ...
        private decimal product = 0;
        private const int adjacentSize = 4;
        ...
        public string Answer()
        {
            for (int col = 0; col < grid.GetLength(0); col++)
            {
                for (int row = 0; row < grid.GetLength(1); row++)
                {
                    decimal tempProduct;
                    //check vertically
                    if (row < grid.GetLength(0) - adjacentSize)
                    {
                        tempProduct = grid[col, row];
                        for (int i = 1; i < adjacentSize; i++)
                        {
                    ...
                    //check horizontally
                    if (col < grid.GetLength(1) - adjacentSize)
                    {
                        tempProduct = grid[col, row];
                        for (int i = 1; i < adjacentSize; i++)
                        {
                    ...
                    //check Diagonally ↗
                    if ((col < grid.GetLength(0) - adjacentSize) && (row >= adjacentSize))
                    {
                        tempProduct = grid[col, row];
                        for (int i = 1; i < adjacentSize; i++)
                        {
                    ...
                    //check diagonally ↘
                    if ((row < grid.GetLength(0) - adjacentSize) && (col < grid.GetLength(1) - adjacentSize))
                    {
                        tempProduct = grid[col, row];
                        for (int i = 1; i < adjacentSize; i++)
                        {
